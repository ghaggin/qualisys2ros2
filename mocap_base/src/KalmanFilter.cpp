...
KalmanFilter::KalmanFilter()
    : attitude(Quaterniond::Identity()), position(Vector3d::Zero()), angular_vel(Vector3d::Zero()),
      linear_vel(Vector3d::Zero()), state_cov(Matrix12d::Identity()),
      input_cov(Matrix12d::Identity()), measurement_cov(Matrix6d::Identity()),
      filter_status(INIT_POSE), last_time_stamp(0.0), msg_interval(0.01),
      proc_jacob(Matrix12d::Zero()), meas_jacob(Matrix6_12d::Zero()),
      proc_noise_jacob(Matrix12d::Zero()), meas_noise_jacob(Matrix6d::Zero()) {
...
void KalmanFilter::update(const Eigen::Quaterniond& m_attitude, const Eigen::Vector3d& m_position) {
    ...
    Vector4d dq_vec4 =
        1.0 / sqrt(1.0 + dq_vec3.squaredNorm()) * Vector4d(dq_vec3(0), dq_vec3(1), dq_vec3(2), 1.0);
